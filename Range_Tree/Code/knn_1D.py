...
from build_1D_Range_Tree import sorted_array_to_bst, is_leaf
...
def nearest_neighbor(root, target_point, axis, nearest_list):
    ...
    if root is None:
        ...
        best_point = nearest_neighbor(nextBranch, target_point, axis, nearest_list)
        ...
        if squared_radius >= absolute_distance**2:
            temp_best = nearest_neighbor(otherBranch, target_point, axis, nearest_list)
...
def k_nn(root, target_point, num_points, k):
    ...
    if k < 1:
        ...
        while (len(nearest_list) < k) and (len(nearest_list) < num_points):
            nn = nearest_neighbor(root, target_point, 0, nearest_list)
...
def main():
    ...
    target_point = (3,3)
    knn = k_nn(root, target_point, num_points=len(lista), k=7)
